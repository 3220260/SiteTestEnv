...
(function () {
  const PROCESS_IDS = ['v-port', 'v-new', 'n-port', 'n-new'];
  const STEP_LABELS = ['Έγγραφα', 'Πληρωμή', 'Email', 'SIM'];
  const STEP_HELP = [
    'Κατέβασε και συμπλήρωσε τα απαραίτητα έντυπα.',
    'Κάνε την κατάθεση και κράτησε το αποδεικτικό.',
    'Στείλε τα έγγραφα και το αποδεικτικό στο email.',
    'Όταν παραλάβεις τη SIM, κάνε την τελική ενεργοποίηση.'
  ];
  ...
  function getSteps(container) {
    if (!container) return [];
    return Array.from(container.querySelectorAll(':scope > .relative.flex'))
      .filter((step) => step.querySelector('h4'));
  }

  function getCurrentIndex(containerId) {
    if (typeof processWizardState === 'object' && processWizardState) {
      return processWizardState[containerId] || 0;
    }

    const container = document.getElementById(containerId);
    const steps = getSteps(container);
    const activeIndex = steps.findIndex((step) => !step.classList.contains('hidden'));
    return activeIndex >= 0 ? activeIndex : 0;
  }

  function getStepTitle(step, index) {
    const fallback = STEP_LABELS[index] || `Βήμα ${index + 1}`;
    const title = step?.querySelector('h4')?.textContent?.trim();
    if (!title) return fallback;
    return title.length > 34 ? fallback : title;
  }

  function createMobileWizard(containerId) {
    const container = document.getElementById(containerId);
    if (!container || container.querySelector('[data-mobile-noob-wizard]')) return;

    const desktopWizard = container.querySelector('[data-process-wizard="true"]');

    const mobileWizard = document.createElement('div');
    mobileWizard.className = 'mobile-noob-wizard';
    mobileWizard.dataset.mobileNoobWizard = 'true';
    mobileWizard.dataset.processContainer = containerId;

    mobileWizard.innerHTML = `
      <div class="mobile-noob-top">
        <span class="mobile-noob-count" data-mobile-noob-count>Βήμα 1/4</span>
        <strong class="mobile-noob-title" data-mobile-noob-title>Έγγραφα</strong>
      </div>

      <p class="mobile-noob-help" data-mobile-noob-help>
        Κατέβασε και συμπλήρωσε τα απαραίτητα έντυπα.
      </p>

      <div class="mobile-noob-progress" aria-hidden="true">
        <span data-mobile-noob-progress></span>
      </div>

      <div class="mobile-noob-chips" data-mobile-noob-chips></div>

      <div class="mobile-noob-actions">
        <button type="button" class="mobile-noob-btn mobile-noob-prev" data-mobile-process-prev>
          Πίσω
        </button>
        <button type="button" class="mobile-noob-btn mobile-noob-next" data-mobile-process-next>
          Επόμενο
        </button>
      </div>
    `;
    ...
    if (desktopWizard) {
      desktopWizard.insertAdjacentElement('afterend', mobileWizard);
    } else {
      container.prepend(mobileWizard);
    }
  }

  function updateMobileWizard(containerId) {
    const container = document.getElementById(containerId);
    if (!container) return;

    createMobileWizard(containerId);

    const mobileWizard = container.querySelector('[data-mobile-noob-wizard]');
    const steps = getSteps(container);
    if (!mobileWizard || !steps.length) return;

    const currentIndex = Math.min(Math.max(getCurrentIndex(containerId), 0), steps.length - 1);
    const currentStep = steps[currentIndex];

    const count = mobileWizard.querySelector('[data-mobile-noob-count]');
    const title = mobileWizard.querySelector('[data-mobile-noob-title]');
    const help = mobileWizard.querySelector('[data-mobile-noob-help]');
    const progress = mobileWizard.querySelector('[data-mobile-noob-progress]');
    const chips = mobileWizard.querySelector('[data-mobile-noob-chips]');
    const prev = mobileWizard.querySelector('[data-mobile-process-prev]');
    const next = mobileWizard.querySelector('[data-mobile-process-next]');

    if (count) count.textContent = `Βήμα ${currentIndex + 1}/${steps.length}`;
    if (title) title.textContent = getStepTitle(currentStep, currentIndex);
    if (help) help.textContent = STEP_HELP[currentIndex] || 'Ακολούθησε το τρέχον βήμα.';
    if (progress) progress.style.width = `${((currentIndex + 1) / steps.length) * 100}%`;
    ...
    if (chips) {
      ...
      steps.forEach((_, index) => {
        ...
        chip.type = 'button';
        chip.className = `mobile-noob-chip${index === currentIndex ? ' is-active' : ''}`;
        chip.textContent = `${index + 1}. ${STEP_LABELS[index] || 'Βήμα'}`;
        chip.dataset.mobileProcessGoto = String(index);
    ...
    if (next) {
      next.disabled = false;
      next.textContent = currentIndex === steps.length - 1 ? 'Ολοκλήρωση' : 'Επόμενο';
    }
  }

  function scrollProcessContentToTop(containerId) {
    if (!isMobile()) return;

    const container = document.getElementById(containerId);
    if (!container) return;

    const scrollRoot = container.closest('.custom-scroll') || container.closest('.modal-backdrop') || document.scrollingElement;
    const activeStep = getSteps(container).find((step) => !step.classList.contains('hidden'));

    if (!scrollRoot || !activeStep) return;

    requestAnimationFrame(() => {
      const mobileWizard = container.querySelector('[data-mobile-noob-wizard]');
      const rootRect = scrollRoot.getBoundingClientRect ? scrollRoot.getBoundingClientRect() : { top: 0 };
      const stepRect = activeStep.getBoundingClientRect();
      const offset = mobileWizard ? mobileWizard.getBoundingClientRect().height + 12 : 12;
      const nextTop = scrollRoot.scrollTop + (stepRect.top - rootRect.top) - offset;
  ...
  function goToStep(containerId, index) {
    if (typeof showProcessWizardStep !== 'function') return;

    showProcessWizardStep(containerId, index);
    updateMobileWizard(containerId);
    scrollProcessContentToTop(containerId);
  }

  function initMobileWizards() {
    PROCESS_IDS.forEach((containerId) => {
      createMobileWizard(containerId);
      updateMobileWizard(containerId);
    });
  }
  ...
  if (typeof showProcessWizardStep === 'function') {
    ...
    showProcessWizardStep = function simpleMobileShowProcessWizardStep(containerId, index) {
      originalShowProcessWizardStep(containerId, index);

      if (PROCESS_IDS.includes(containerId)) {
        updateMobileWizard(containerId);
      }
    };
  }
  ...
  document.addEventListener('click', function (event) {
    const mobileControl = event.target.closest(
      '[data-mobile-process-prev], [data-mobile-process-next], [data-mobile-process-goto]'
    );
    ...
    const mobileWizard = mobileControl.closest('[data-mobile-noob-wizard]');
    const containerId = mobileWizard?.dataset.processContainer;
    if (!containerId) return;
    ...
    const currentIndex = getCurrentIndex(containerId);

    if (mobileControl.matches('[data-mobile-process-goto]')) {
      goToStep(containerId, Number(mobileControl.dataset.mobileProcessGoto));
      return;
    }

    if (mobileControl.matches('[data-mobile-process-prev]')) {
      goToStep(containerId, Math.max(0, currentIndex - 1));
      return;
    }

    if (mobileControl.matches('[data-mobile-process-next]')) {
      if (currentIndex === steps.length - 1) {
        const modal = container.closest('.modal-backdrop');
        if (modal?.id && typeof closeModal === 'function') closeModal(modal.id);
        return;
      }

      goToStep(containerId, Math.min(steps.length - 1, currentIndex + 1));
    }
  }, true);

  document.addEventListener('DOMContentLoaded', initMobileWizards);
  window.addEventListener('resize', initMobileWizards);
})();
